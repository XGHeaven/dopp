...
export const create: ServiceCreator<MongoServiceConfig, MongoServiceOptions> = (
  ...
  async function evalRootService(evalString: string) {
    await runComposeCommand(await getServiceApp(), ['exec', 'default', 'mongo', '-u', 'root', '-p', await ctx.getConfig('rootPassword', ''), '--eval', evalString], false)
  }

  async function createConnn(conn: string, options: Partial<MongoServiceConnConfig>) {
    ...
    await evalRootService(`db.getSiblingDB('${database}').createUser(${JSON.stringify({
      user: username,
      pwd: password,
    ...
    await ctx.setConfig('conns', {...conns, [conn]: {username, password, database}})
  }
  ...
  async function removeConn(conn: string) {
    ...
    await evalRootService(`db.getSiblingDB('${database}').dropUser('${username}')`)
  ...
  async function enterREPL(conn?: string) {
    ...
    await runComposeCommand(app, ['exec', 'default', 'mongo', '-u', username, '-p', password, database], false)
  }
    ...
    command(yargs: Yargs.YargsType) {
      return ctx.registeProcessCommand(
        yargs
          ...
          .command("create <conn>", "Create connection", (_yargs: Yargs.YargsType) => _yargs.option('username', {
            type: 'string',
            alias: ['u'],
          ...
          }), ({ conn, username, password, database }: any) => createConnn(conn, {username, password, database}))
          .command('remove <conn>', 'Remove connection', () => {}, ({conn}: any) => removeConn(conn))
          .command('repl [db]', "Enter repl of db", () => {}, ({db}: any) => enterREPL(db))
